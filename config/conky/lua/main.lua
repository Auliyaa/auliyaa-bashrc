...
function draw_clock(cr)
	local datestr = conky_parse('${exec date +"%A %B %Y"}')
	local timestr = conky_parse('${exec date +"%H:%M:%S"}')

	-- draw time
	text_set_font(cr, font, 42, CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD)
	local dtw,dth = text_dimensions(cr, timestr)
	text_draw(cr, timestr, (w-dtw)/2, dth+25, color_1, 1)

	text_set_font(cr, font, 22)
	local ttw,tth = text_dimensions(cr, datestr)
	text_draw(cr, datestr, (w-ttw)/2, dth+35+tth+20, color_1, 1)
end
